...
analysis_forecast1 = st.sidebar.selectbox('Analysis or Forecast:',('Analysis','Forecast'))
...
entire_day = st.sidebar.selectbox('Graph Entire Day/Forecast (Takes a Bit):',('No','Yes'))
...
wind_mph=projected.sel(x=lon, y=lat, method="nearest")[variable].values*2.23694
...
# projected2=projected[variable].values
# projected2=projected2[rows-size:rows+size,columns-size:columns+size]
# img=projected2*2.23694
    ...
    if analysis_forecast=='anl':
        ...
        def get_data_date_time(date_time,lat,lon):
            ...
            try:
                ds = xr.open_mfdataset([lookup(path), lookup(f"{path}/{vertical_level}")], 
                   engine="zarr") 
                result=ds.sel(projection_x_coordinate=lon2, projection_y_coordinate=lat2, method="nearest")[variable].values*2.23694
            except:
                result=None
        ...
        result=ds_forecast[variable].sel(projection_x_coordinate=lon2, projection_y_coordinate=lat2, method="nearest")*2.23694
